...
namespace SSOApp.Controllers.UI
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            // Setup Default Admin and Roles.
            bool IsCreateDefaultRoles = _config.GetValue<bool>("DefaultRoles");
            bool IsCreateDefaultAdmin = _config.GetValue<bool>("DefaultAdmin");
            if (IsCreateDefaultRoles)
                await SetupApp.SetupRolesAndAdmin();
            if (IsCreateDefaultAdmin)
                await SetupApp.SetupAdmin();
        ...
        public async Task<IActionResult> Login(LoginInputModel model, string button)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                int loginerror = 1;
                var count = UserHandler.ConnectedIds.FirstOrDefault(d => d.UserName == model.Username);
                var checkscreens = 0;
                if (count != null)
                {
                    checkscreens = count.Count;
                }
                if (checkscreens <= SetupApp.UserScreens)
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        using (var client = new HttpClient())
                        {
                            client.BaseAddress = new Uri("https://localhost:44391/APIUser/savelastlogintime?username=" + user.UserName);
                            var postTask = await client.GetAsync(client.BaseAddress);
                            string apiResponse = await postTask.Content.ReadAsStringAsync();
                            //postTask.Wait();
                            var resultjson = JsonConvert.DeserializeObject<APIReturnedModel>(apiResponse);
                ...
                if (loginerror == 2)
                {
                    await _events.RaiseAsync(new UserLoginFailureEvent(model.Username, "Max screen", clientId: context?.ClientId));
                    ModelState.AddModelError(string.Empty, AccountOptions.InvalidCredentialsMaxScreen);
                }
                else
                {
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string button)
        {
            ...
            else
            {
                if (ModelState.IsValid)
                {
                    using (var client = new HttpClient())
                    {
                        client.BaseAddress = new Uri("https://localhost:44391/AppAccount");
                        ...
                        var stringContent = new StringContent(json, UnicodeEncoding.UTF8, "application/json");
                        //HTTP POST                        
                        var postTask = await client.PostAsync("AppAccount", stringContent);
